...
def compute_confidence_advantage(data: DataProto, adv_estimator, norm_adv_by_std_in_grpo=True):
...
class RayConstraintTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        metric_dict['result/avg_confidence'] = np.mean(confidences)
        metric_dict['result/avg_brier_score'] = np.mean(brier_scores)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if False and self.use_constraints and self.constraint_manager is not None:
                            ...
                            metrics.update({"critic/original_rewards/mean": new_batch.batch['original_token_level_scores'].mean().item()})

...
class CalashRewardManager:
    """
    Manages Lagrangian constraints for DAPO, adapted for Confidence Calibration (CALASH).
    
    This class implements CALASH (CALibration-Adaptive SHaping).
    ...
    def __init__(
        ...
        if constraint_type != "average":
            raise ValueError("CALASH only supports 'average' constraint on Brier score.")
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        if self.stats["avg_brier"]:
            metrics["calash/avg_brier"] = self.stats["avg_brier"][-1]
            metrics["calash/std_brier"] = self.stats["std_brier"][-1]
            metrics["calash/min_brier"] = self.stats["min_brier"][-1]
            metrics["calash/max_brier"] = self.stats["max_brier"][-1]
            metrics["calash/avg_confidence"] = self.stats["avg_confidence"][-1]
            metrics["calash/avg_task_reward"] = self.stats["avg_task_reward"][-1]

            # Constraint satisfaction metrics
            metrics["calash/satisfaction_rate"] = self.stats["satisfaction_rate"][-1]
            metrics["calash/num_violations_rate"] = self.stats["num_violations"][-1]
            
            # Distance from target
            metrics["calash/avg_violation"] = self.stats["constraint_violation"][-1]
        ...
        if self.stats["lagrange_multiplier"]:
            metrics["calash/lambda"] = self.stats["lagrange_multiplier"][-1]
            if self.stats["lambda_change_rate"]:
                metrics["calash/lambda_change_rate"] = self.stats["lambda_change_rate"][-1]
                
        # Add current target
        metrics["calash/target_brier_score"] = self.target_brier_score
    ...
    def get_summary_string(self) -> str:
        ...
        if "calash/avg_brier" in metrics:
            summary_parts.append(
                f"Avg Brier: {metrics['calash/avg_brier']:.3f} "
                f"(target: {self.target_brier_score:.3f}, "
                f"sat_rate: {metrics.get('calash/satisfaction_rate', 0):.2%})"
            )
        
        if "calash/avg_confidence" in metrics:
            summary_parts.append(f"Avg Conf: {metrics['calash/avg_confidence']:.3f}")
            
        if "calash/lambda" in metrics:
            summary_parts.append(f"λ: {metrics['calash/lambda']:.4f}")
            
        if "calash/avg_penalty" in metrics:
            summary_parts.append(
                f"Penalty: {metrics['calash/avg_penalty']:.3f} "
                f"(active: {metrics.get('calash/penalty_active_rate', 0):.2%})"
            )
            
        if "calash/avg_task_reward" in metrics:
            summary_parts.append(f"Task R: {metrics['calash/avg_task_reward']:.3f}")
    ...
    def save_state(self, file_path: str):
        ...
        try:
            ...
            print(f"\033[92m✅ CALASH Manager state saved to {file_path}\033[0m")
        except Exception as e:
            print(f"\033[91m❌ Error saving CALASH Manager state: {e}\033[0m")
    ...
    def load_state(self, file_path: str):
        ...
        if not os.path.exists(file_path):
            print(f"\033[93m⚠️ [WARNING] CALASH checkpoint file not found at {file_path}. Starting with a fresh state.\033[0m")
        ...
        try:
            ...
            print(f"\033[92m✅ CALASH Manager state loaded from {file_path}\033[0m")
            print(f"  - Resumed λ: {self.lagrange_multiplier:.4f}")

        except Exception as e:
            print(f"\033[91m❌ Error loading CALASH Manager state from {file_path}: {e}. \033[0m")
            # Don't raise error, just warn and continue with fresh state
            print(f"\033[93m⚠️ [WARNING] Proceeding with a fresh CALASH state.\033[0m")